...
from jwt_tokens.token_handlers import create_access_token, create_access_token_payload
...
async def authenticate_user(user: User, password: str, session: AsyncSession):
    if "@" in password:
        user_login = await email_exist(user.email, session)
    else:
        user_login = await username_exist(user.username, session)
